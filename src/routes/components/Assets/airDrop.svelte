        ...
        <div class="flex justify-between my-4">
          ...
          <div class="mt-5">
            <span>{item.amount}</span>
              <Button color="blue" size="xs" on:click={()=>{checkTrust({code:item.asset.split(":")[0], issuer:item.asset.split(":")[1]}) ? createClaimTransaction({asset: item.asset,balanceID:item.id,amount:item.amount}): createTrustNClaimTransaction({asset: item.asset,balanceID:item.id,amount:item.amount})}}>
                {#if isProcessing}
